...
class PXSolver:
    ...
    def gen_px_authorization(self):
        ...
        Returns:
            tuple: (auth_string, uuid)
        """
        fingerprint = self.__get_fingerprint()
        auth, uid = self.gen_px(fingerprint)
        return auth, uid
    ...
    def gen_px(self, fingerprint):
        ...
        first_req = self.session.post(
            "https://collector-pxrf8vapwa.perimeterx.net/api/v1/collector/mobile",
            data=data
        )
        if first_req.status_code != 200:
            raise PerimeterXError(
                f"Error while posting first payload, code {first_req.status_code} message : {first_req.text}"
            )

        resp_data = first_req.json()['do']
        vid = None
        sid = None
        appc = None
        ...
        if not vid or not sid or not appc:
            raise PerimeterXError(f"Cannot find vid, sid or appc. Data : {resp_data}")
        ...
        if first_req.status_code != 200:
            raise PerimeterXError(
                f"Error while posting second payload, code {first_req.status_code} message : {first_req.text}"
            )

        resp_data = first_req.json()['do']
        if len(resp_data) != 1 or resp_data[0].split("|")[0] != "bake":
            raise PerimeterXError(f"Error parsing PX response: {resp_data}")

        args = resp_data[0].split("|")
